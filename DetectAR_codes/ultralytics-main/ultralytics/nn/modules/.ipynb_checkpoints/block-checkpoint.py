...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, ECA, CBAM
from .transformer import TransformerBlock

__all__ = ('DFL', 'HGBlock', 'HGStem', 'SPP', 'SPPF', 'C1', 'C2', 'C3', 'C2f', 'C3x', 'C3TR', 'C3Ghost',
           'GhostBottleneck', 'Bottleneck', 'BottleneckCSP', 'Proto', 'RepC3', 'C2fECA')
...
class C2fECA(nn.Module):
    """C2f with Deformable Convolution"""
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super(C2fECA, self).__init__()
        self.c = int(c2 * e)
        self.cv1 = nn.Conv2d(c1, 2 * self.c, 1, 1)  # 初始卷积层
        self.cv2 = nn.Conv2d((2 + n) * self.c, c2, 1)  # 输出卷积层
        self.m = nn.ModuleList(Bottleneck_DCN(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))  # 使用 Bottleneck_DCN 代替标准卷积
        # self.eca = CBAM(c2)  # ECA 注意力模块
...
class DCNv2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, dilation=1, groups=1, deformable_groups=1):
        ...
        self.act = nn.ReLU()  # 你可以根据需要选择不同的激活函数
...
class Bottleneck_DCN(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        if k[1] == 3:
            ...
            self.cv2 = nn.Conv2d(c_, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2
